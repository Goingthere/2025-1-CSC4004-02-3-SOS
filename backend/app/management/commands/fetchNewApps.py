        ...
        try:
            ...
            app, _ = AppInfo.objects.update_or_create(
                ...
                defaults={
                    ...
                    "currentlyDiscounted": currently_discounted,
                }
            )
            ...
            # 할인 이력 데이터 수집 함수 호출
            self.getItadDiscountHistory(app)

        except Exception as e:
            self.stderr.write(f"App {appid} 저장 실패: {e}")
            
    def getItadDiscountHistory(self, appObj):
        apiKey = loadApiKey()
        appID = appObj.appID
